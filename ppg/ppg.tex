...
\item This allows us to use it to find the kth element in an array. The following algorithm is called quick select:
\begin{lstlisting}
//where kth largest element of a is to be found.
int pq, q, e,n;
n = a.size();
pq = 0;
q = 0;
...
while(q<n-k){
    pq = q;
    q = partition(a,q,e);
    //repeatedly pick the right partition, while the kth element is still contained in it.
    if(pq==q){break;}
}
sort(a.begin()+pq,a.begin()+n);
return a[n-k];
\end{lstlisting}
\end{itemize}
