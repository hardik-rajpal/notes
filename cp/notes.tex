...
\hbadness 100001
\begin{document}
\lstset{language=C++,backgroundcolor=\color{Lavender},
                basicstyle=\ttfamily,
...
\begin{itemize}
    ...
    \citem{iter lower\_bound(first,last,value)} (Returns \texttt{iter} to the smallest element $\geq$ \texttt{value}.) (O(logn))
    \citem{iter upper\_bound(first,last,value)} (Returns \texttt{iter} to the smallest element $>$ \texttt{value}.) (O(logn))
\end{itemize}
