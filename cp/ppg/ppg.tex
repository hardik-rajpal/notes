...
\begin{itemize}
\item They are aka height-balanced trees.
\item They are binary (searchable,sorted) trees, with the added property of balance.
\item If $h(n)$ denotes the height of the tree rooted at n:
\begin{center}
    $h(n) = 1+max_{u \in n.children}(h(u))$\\
    $h(n) = 1 \implies $ n is a leaf.\\
\end{center}
\item $\forall n \in \{Internal Nodes\}, |h(u)-h(v)| \le 1$, where $u$ and $v$ are children of n.
